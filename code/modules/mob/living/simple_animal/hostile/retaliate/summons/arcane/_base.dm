/mob/living/simple_animal/hostile/retaliate/rogue/arcane
	obj_damage = 75
	speak_emote = list("crackles")
	can_mine = TRUE

/mob/living/simple_animal/hostile/retaliate/rogue/arcane/Initialize()
	. = ..()
...
/mob/living/simple_animal/hostile/retaliate/rogue/elemental/simple_limb_hit(zone)
	...
	return ..()
*/
/mob/living/simple_animal/hostile/retaliate/rogue/arcane/simple_add_wound(datum/wound/wound, silent = FALSE, crit_message = FALSE)	//no wounding the elementals
	return

/mob/living/simple_animal/hostile/retaliate/rogue/arcane/attackby(obj/item/I, mob/living/carbon/human/user, params)
	if(istype(I, /obj/item/magic))
		var/obj/item/magic/magicmaterial = I
		if(istype(magicmaterial, /obj/item/magic)) // healed by any arcyne stuff
			if(health == maxHealth)
